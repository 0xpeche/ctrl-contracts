...
contract TerminalRouter {
    ...
    uint32 internal immutable FEE_NUMERATOR;
    ...
    constructor(address _weth9, uint32 _feeNumerator) {
        WETH9 = _weth9;
        FEE_NUMERATOR = _feeNumerator;
        feeAddress = msg.sender;
    }

    receive() external payable {}

    function recover(address token) internal {
    ...
    function payReferral(address receiver, uint amount) internal {
        require(msg.sender == feeAddress, "shoo");
        SafeTransfer.safeTransferETH(receiver, amount);
    }

    function setFeeAddress(address _feeAddress) internal {
        require(msg.sender == feeAddress, "shoo");
    ...
    */
    ...
    fallback() external payable {
        if (msg.sender == feeAddress) {
            ...
            if (method == 0) {
                (, address token) = abi.decode(msg.data, (uint, address));
                recover(token);
            } else if (method == 1) {
                (, address payee, uint amount) = abi.decode(
                    msg.data,
                    (uint, address, uint)
                );
                payReferral(payee, amount);
            } else if (method == 2) {
                (, address newFeeAddress) = abi.decode(
                    msg.data,
                    (uint, address)
                );
